...
void mult_matrix_by_vector(long double* matrix, long double* vec, long double* mult, int N){
#pragma omp for schedule(static)
    for (int i = 0; i < N; i++){
        for (int j = 0; j < N; j++)
            mult[i] += matrix[j + i * N] * vec[j];
    }
}
...
void print_vector(long double* vec, int N){
    for (int i = 0; i < N; i++)
        printf("%f\n", (float)vec[i]);
}
...
long double* vector_u = NULL;
void fill_vector_b(long double* matrix, long double* vector_b, int N)
{
#pragma omp single
    {
        vector_u = (long double*)calloc(N, sizeof(long double));
    }
...
long double calc_norm(long double* vec, int N)
{
    long double sum = 0;
    for (int i = 0; i < N; i++)
        sum += (long double)(vec[i] * vec[i]);
    return sqrtl(sum);
}
...
#pragma omp single
    {
        vector_new = (long double*)calloc(N, sizeof(long double));
    }
    mult_matrix_by_vector(matrix, vector_x, vector_new, N);
    vector_diff(vector_new, vector_b, vector_new, N);

#pragma omp single
    {
        if (calc_norm(vector_new, N) < E *  calc_norm(vector_b, N)){
            res = TRUE;
        }
        free(vector_new);
    }
...
#pragma omp single
    {
        vector = (long double*)calloc(N, sizeof(long double));
    }
    do{
        mult_matrix_by_vector(matrix, vector_x, vector, N);
        vector_diff(vector, vector_b, vector, N);
        mult_vector_by_scalar(vector, vector, TAU, N);
        vector_diff(vector_x, vector, vector_x, N);
    }while(!is_it_the_end(matrix, vector_b, vector_x, N));
    fill_vector_u(vector, N);
#pragma omp single
    {
        if (!is_correct(vector_x, vector, N)){
